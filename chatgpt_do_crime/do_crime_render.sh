...
# Clean stock footage: no publisher overlays, captions, channel bugs or watermarks.
...
# Actual protest footage hosted by Wikimedia Commons.
...
# High-resolution FARC-themed reconstruction frames made during the source pass.
...
# Exact Tout Va Bien supermarket frame from the requested sequence; original-size Blogger asset.
...
# Exact supplied track via the artist's Audiomack page.
yt-dlp --no-playlist --retries 5 --socket-timeout 45 -f 'bestaudio/best' -x --audio-format m4a --audio-quality 0 -o 'work/audio/raya_source.%(ext)s' 'https://audiomack.com/zericxxn/song/raya-slowed'
...
# Preserve source-size evidence before reframing.
{
  for f in work/raw/cash.mp4 work/raw/luxury.mp4 work/raw/cyber.mp4 work/raw/snow.mp4 work/raw/flag.webm; do
    echo "=== $f ==="; ffprobe -v error -select_streams v:0 -show_entries stream=codec_name,width,height,r_frame_rate:format=duration -of default=nw=1 "$f" || true
  done
  python - <<'PY'
...
# Normalize video sources to portrait. Crop centers aggressively so no edge overlay can survive.
norm() {
  key="$1"; in="$2"; x="$3"; top="$4"; bottom="$5"
  ffmpeg -y -v warning -i "$in" -an -vf "crop=iw*0.54:ih*(1-${top}-${bottom}):(iw-iw*0.54)*${x}:ih*${top},scale=1080:1920:flags=lanczos,setsar=1,fps=60,format=yuv420p" -t 18 -c:v libx264 -preset veryfast -crf 18 -profile:v high "work/src/${key}.mp4"
}
...
# Animate high-resolution stills with slow opposing Ken Burns moves, then concatenate.
...
# Clean title cards: grain/noise is applied before these overlays, never after.
python - <<'PY'
from pathlib import Path
from PIL import Image,ImageDraw,ImageFont
W,H=1080,1920; rows=12; rh=H//rows
font_candidates=[Path('/usr/share/fonts/opentype/didot/GFSDidotBold.otf'),Path('/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf')]
...
# 55 equal 110-BPM eighth-note slots in exactly 15 s. Rotate every requested motif.
python - <<'PY'
from pathlib import Path
S=3/11; N=55
cats=['cash','luxury','cyber','snow','farc','tout','flag','cyber','cash','snow','farc','tout','luxury','flag','cyber','snow','cash','farc','tout']
idx={'cash':0,'luxury':1,'cyber':2,'snow':3,'farc':4,'tout':5,'flag':6}
counts={k:0 for k in idx}; speed=[.72,1.33,2.55,.61,1.82,2.95,1.0,2.18,.84,1.56,2.72,.68]
parts=[]; labels=[]
for i in range(N):
 cat=cats[i%len(cats)]; c=counts[cat]; counts[cat]+=1
 cue=(.25+c*1.07)%9.7; sp=speed[i%len(speed)]; need=S*sp
 zoom=1.035+(i%5)*.014; sw=round(1080*zoom); sh=round(1920*zoom); x=max(0,(sw-1080)//2+((i%3)-1)*11); y=max(0,(sh-1920)//2+((i%4)-2)*7)
 chain=f'[{idx[cat]}:v]trim=start={cue:.5f}:end={cue+need:.5f},setpts=(PTS-STARTPTS)/{sp:.5f},fps=60,scale={sw}:{sh}:flags=bicubic,crop=1080:1920:{x}:{y}'
 if i in {6,15,28,42,49}: chain+=',loop=loop=3:size=2:start=2,trim=duration=0.272727,setpts=PTS-STARTPTS'
 chain+=f',format=yuv420p[s{i}]'; parts.append(chain); labels.append(f'[s{i}]')
parts.append(''.join(labels)+f'concat=n={N}:v=1:a=0[base]')
# Victory-style abuse: high contrast, oversaturation, RGB separation, VHS/noise, scanlines.
parts.append("[base]eq=contrast=1.31:saturation=1.48:brightness=-0.035,rgbashift=rh=12:bh=-12:rv=3:bv=-3,noise=alls=12:allf=t+u,drawgrid=w=1080:h=4:t=1:c=black@0.28,unsharp=5:5:0.9:5:5:0[fx0]")
...
# Three independently offset tear strips, gated to beat-adjacent windows.
parts.append('[fx1]split=4[m][a][b][c]')
parts.append('[a]format=rgba,crop=1080:105:0:335,pad=1080:1920:0:335:color=black@0[ta]')
parts.append('[b]format=rgba,crop=1080:88:0:915,pad=1080:1920:0:915:color=black@0[tb]')
parts.append('[c]format=rgba,crop=1080:125:0:1435,pad=1080:1920:0:1435:color=black@0[tc]')
e1='+'.join(f'between(t,{t:.4f},{t+.080:.4f})' for t in [2.18,5.18,8.18,11.18,13.64]); e2='+'.join(f'between(t,{t:.4f},{t+.065:.4f})' for t in [3,6,9,12]); e3='+'.join(f'between(t,{t:.4f},{t+.070:.4f})' for t in [4.09,7.09,10.09,13.09])
parts.append(f"[m][ta]overlay=x=72:y=0:enable='{e1}'[o1]");parts.append(f"[o1][tb]overlay=x=-58:y=0:enable='{e2}'[o2]");parts.append(f"[o2][tc]overlay=x=86:y=0:enable='{e3}'[o3]")
# Title cards, added LAST and therefore perfectly clean/no grain.
prev='o3'; title_inputs=[8,9,10,11]; title_slots=[0,20,40,54]
for n,(inp,slot) in enumerate(zip(title_inputs,title_slots),1):
...
# Inputs 0-6 footage; input 7 audio; 8-11 clean title frames.
